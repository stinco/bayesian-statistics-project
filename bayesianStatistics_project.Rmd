...
As for the previous model, the overlay plot and the statistics computed on the whole dataset the model looks fine.
...
That means that, even considering the time as an explanatory variable, `mean.temp` has a significative effect. Looking to the marginal effect plot for both the model (in the previous chapter) we can see that the mortality is higher when it is really cold and when it is really hot. On the contrary, `rel.humid` has not a significative effect on mortality.

As regards the pollution variables (`SO2` and `TSP`), we can see that `TSP` has not a significative effect on mortality, while `SO2` has a significative effect if we consider only the death for respiratory issues. Furthermore, the `SO2` coefficient is positive, so the hypothesis that the concentration of sulphur dioxide has an impact on respiratory death is supported by the data. However, as soon as the respiratory issues are just a small part of the whole cause of death, it doesn't result significative on the mortality in general.
...
A great improvement for the model would be obtained by obtaining new variables as the cause of death and the age at death. With the cause of death, we could build a model for each cause and then ensemble them to built a more accurate model. With the age at death we could build models for specific mortality rate and take into account the age distribution.
...
To deal with the outliers we could also consider other kind of models, like the quasi-Poisson and the negative binomial. I tried with it, but I had trouble with the non-convergence of the Markov Chains.
